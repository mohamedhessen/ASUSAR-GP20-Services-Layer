...
*/
    ...
    closeGraphStatusClicked : function (e) {
        if (this.$.rovGraphStatusDialog.opened) {
            this.$.rovGraphStatusDialog.close();
        }
    },
    ...
    evalResult : function (data) {
        ...
        if (count == 0) {
            ...
            this.rovGraphStatus(status.length > 0 ? status : null);
        }
        else {
    ...
    getViewCallback: function (error, viewData, module, view, handle) {
        ...
        if (this.requests == 0) {
            if (this.requestErrors.length == 0) {
                ...
                this.rovGraphStatus(this.requestErrors);
            }
        }
    },
    ...
    onRefresh: function () {
        ...
        if (this.requestErrors.length > 0 && this.$.rovGraphStatusDialog.opened) {
            this.$.rovGraphStatusDialog.close();
        }
        this.requestErrors = [];
        var requests = [];
    ...
    */
    ...
    rovGraphStatus : function (status) {
        if (status == null) {
            if (this.$.rovGraphStatusDialog.opened) {
                this.$.rovGraphStatusDialog.close();
            }
            ...
            for (var i = 0; i < status.length; i++) {
                width = rovUtils.getStringWidth(status[i], '12pt arial');
                if (width > maxWidth) {
                    maxWidth = width;
                }
            }
            this.$.rovGraphStatusDialog.style.width = maxWidth + 'px';
            this.set('graphStatus', status);
            if (!this.$.rovGraphStatusDialog.opened) {
                this.$.rovGraphStatusDialog.resetFit();
                var left = this.offsetLeft;
                this.$.rovGraphStatusDialog.style.left = this.rovPanel.isNarrow() ?
                                                         this.offsetLeft + 'px': (this.offsetLeft + rovUtils.leftPaneWidth) + 'px';
                this.$.rovGraphStatusDialog.style.top = (56 + this.offsetTop) + 'px';
                this.$.rovGraphStatusDialog.open();
            }
        }
    },

    rovGraphStatusDialogClosed : function (e) {
        if (e.target.id == 'rovGraphStatusDialog') {
            this.set('graphStatus', []);
        }
    },
    ...
    tracePropertiesDialogOkClicked : function (e) {
        ...
        if (!this.selectedTrace.args && this.selectedTrace.viewType != 'MODULE') {
            ...
            if (this.selectedTrace.customTable) {
                var defaultName = this.selectedTrace.viewName + ' ' + this.selectedTrace.displayColumnName;
            }
            else if (this.selectedTrace.args) {
                var defaultName = (this.selectedTrace.view == 'Variable') ? this.selectedTrace.firstArg : this.selectedTrace.selectedColumnName;
            }
            else {
                var defaultName = this.selectedTrace.selectedColumnName;
            }
