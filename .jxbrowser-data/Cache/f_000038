...
*/
   ...
   /*
    *  ======== closeTableStatusClicked ========
    */
    closeTableStatusClicked : function (e) {
        if (this.$.rovTableStatusDialog.opened) {
            this.$.rovTableStatusDialog.close();
        }
    },
    ...
    */
    renderFinalData : function () {
        ...
        this.rovTableStatus(status.length > 0 ? status : null);
        var tableData = this.formatTableData();
    ...
    *  ======== rovTableStatus ========
    */
    rovTableStatus : function (status) {
        if (status == null) {
            if (this.$.rovTableStatusDialog.opened) {
                this.$.rovTableStatusDialog.close();
            }
            ...
            this.$.rovTableStatusDialog.style.width = maxWidth + 'px';
            this.set('tableStatus', status);
            this.$.rovTableStatusDialog.resetFit();
            var left = this.offsetLeft;
            this.$.rovTableStatusDialog.style.left = this.rovPanel.isNarrow() ? this.offsetLeft + 'px': (this.offsetLeft + rovUtils.leftPaneWidth) + 'px';
            this.$.rovTableStatusDialog.style.top = (56 + 76 + this.offsetTop) + 'px';
            if (!this.$.rovTableStatusDialog.opened) {
                this.$.rovTableStatusDialog.open();
            }
        }
    },
