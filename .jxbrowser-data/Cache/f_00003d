Polymer({ is : "ti-rov-table",
    ...
    highlightCell : function (rowKeys) {
        if (this.isInstanceView) {
            var id = rowKeys.selectedColumnName;
            var row = 0;
            if (rovUtils.isArray(this.viewData)) {
                var colIndex = -1
                var columns = this.instanceTable.columns;
                for (var i = 0; i < columns.length; i++) {
                    if (columns[i] == rowKeys.primaryKeyColumnName) {
                        colIndex = i;
                        break;
                    }
                }
                var rows = this.instanceTable.rows;
                for (var i = 0; i < rows.length; i++) {
                    if (rows[i][colIndex].val == rowKeys.primaryKeyColumnValue) {
                        row = i;
                        break;
                    }
                }
            ...
            var rowVals = this.instanceDataTable.rows[0].rowVals;
            for (var i = 0; i < rowVals.length; i++) {
                if (rowVals[i][colIndex].val == rowKeys.primaryKeyColumnValue) {
                    row = i;
                    break;
                }
            }
            ...
            var id = rowKeys.displayColumnName;
            var colIndex = -1
            var columns = this.customTable.columns;
            for (var i = 0; i < columns.length; i++) {
        ...
        if (this.highlightedCell != null) {
            /* if a cell is already highlighted, unhighlight it */
            if (this.isAttached && this.clientWidth > 0) {
                this.highlightedCell.elem.style.backgroundColor = this.highlightedCell.prevBackgroundColor;
            }
            this.highlightedCell = null;
        }
        var elem = this.$$('#' + id);
        if (elem) {
            this.highlightedCell = {elem: elem, prevBackgroundColor: elem.style.backgroundColor};
